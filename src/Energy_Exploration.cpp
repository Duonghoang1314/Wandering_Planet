...
void setup() {
  ...
  // --- Function ---
  moveMotor(30, 27, 1000);
  Trace_to_Right_Junction_B(); // Đi thẳng đến ngã rẽ phải
  moveMotor(30, 25, 200);
  turnRight_B();
...
void readSensors_B(){
  ...
  s1 = (sensor[0] > thresholdS1B);
  s2 = (sensor[1] > thresholdS2B);
  s3 = (sensor[2] > thresholdS3B);
  s4 = (sensor[3] > thresholdS4B);
}
...
// --- BLACKSIDE LINE TRACKING FUNCTIONS ---
// --- Trace to Cross T Junction Functions ---
// Read sensors continuously until both s1 and s4 are true(indicating a cross junction)
// Then exit the function
